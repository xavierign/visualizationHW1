...
## Introduction
This study analyzes the data from the initial survey conducted at the beginning of the course "Exploratory Data Analysis and Visualization" at Columbia University. Its proupose, as the team members agreed, is to to do a hands-on work with the visualization tools introduced. We will present with an original visualization answers to some questions about the data. To draw some of the charts, we copied the format from [fivethirtyeight.com](http://fivethirtyeight.com/), which is a great data-analysis and visualization site that we took as inspiration. Also, the colors used in the charts are the same as those google docs. This could be easly implemented using the package [ggthems](https://cran.r-project.org/web/packages/ggthemes/vignettes/ggthemes.html) as a complement of [ggplot2](http://ggplot2.org/). We strongly recomend that you take a look at them if you don't know them already, to avoid changing manually: the font, the sizes, the colors, etc. in each graph individually in your future presentations.

## Who we are
In order to understand the survey responders better, an interactive visual graph was developed using d3 to show the different percentages in terms of genders. Clearly, the number of male students (71%) is more than female (28%). Moreover, male students are dominating the population for each program. It is worth mentioning that the highest percentage of female is within Master in Statistics Program with around 47%. On the other hand, female students has the lowest percentage in Data Science Program (both for master and certification). Overall, the percentages can be summarized in a symmetric graph as follows. An interesting interactive chart can be found [here](http://aa3775.neocities.org/edav.html).
...
<br>
<br>
This is the end of the presentation. We hope you take something usefull from it.
<br>
<br>
