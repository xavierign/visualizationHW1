...
c <- ggplot(data=d[1:10,], aes(x=word2, y=freq)) +
    geom_bar(stat="identity")+ theme_fivethirtyeight() + ggtitle("Most frequently mentioned tools") + scale_fill_gdocs() + scale_color_gdocs() 
c
...
bp3 <- ggplot(plot_data4, aes(x=graphics.basics, y=reproducible.documentation, size=Count, label = Count),guide=FALSE)+
  ...
  theme_bw() + scale_x_discrete(limits=c("None","A little","Confident", "Expert")) + scale_y_discrete(limits=c("None","A little","Confident", "Expert")) + theme_fivethirtyeight() + theme(axis.title=element_text(size=12)) + ggtitle("R vs. Matlab") + scale_fill_gdocs() + scale_color_gdocs() + ylab("R data manipulation & modelling")+ theme(legend.position="none")
